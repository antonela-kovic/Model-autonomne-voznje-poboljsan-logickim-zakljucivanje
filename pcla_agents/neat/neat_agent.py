...
class MultiTaskAgent(autonomous_agent.AutonomousAgent):
    ...
    def setup(self, path_to_conf_file):
        ...
        self.net = AttentionField(self.config, 'cpu') # BILO CUDA
        ...
        self.net.encoder.load_state_dict(
            torch.load(os.path.join(path_to_conf_file, 'best_encoder.pth'), map_location='cpu')
        )
        self.net.decoder.load_state_dict(
            torch.load(os.path.join(path_to_conf_file, 'best_decoder.pth'), map_location='cpu')
        )
    ...
    def run_step(self, input_data, timestamp, vehicle=None):
        ...
        if self.step < self.config.seq_len:
            # rgb = torch.from_numpy(scale_and_crop_image(Image.fromarray(tick_data["rgb"])).copy()).unsqueeze(0)
            rgb_array = scale_and_crop_image(Image.fromarray(tick_data['rgb'])).copy()
            rgb = torch.from_numpy(rgb_array).unsqueeze(0)
            self.input_buffer['rgb'].append(rgb.to('cpu', dtype=torch.float32))  # BILO CUDA

            rgb_left_array = scale_and_crop_image(Image.fromarray(tick_data['rgb_left'])).copy()
            rgb_left = torch.from_numpy(rgb_left_array).unsqueeze(0)
            self.input_buffer['rgb_left'].append(rgb_left.to('cpu', dtype=torch.float32))  # BILO CUDA

            rgb_right_array = scale_and_crop_image(Image.fromarray(tick_data['rgb_right'])).copy()
            rgb_right = torch.from_numpy(rgb_right_array).unsqueeze(0)
            self.input_buffer['rgb_right'].append(rgb_right.to('cpu', dtype=torch.float32))  # BILO CUDA
        ...
        gt_velocity = torch.FloatTensor([tick_data['speed']]).to('cpu', dtype=torch.float32) # BILO CUDA
        command = torch.FloatTensor([tick_data['next_command']]).to('cpu', dtype=torch.float32) # BILO CUDA
        target_point = torch.stack(tick_data['target_point']).to('cpu', dtype=torch.float32) # BILO CUDA
    ...
    def save(self, tick_data):
        ...
        for i in range(self.config.tot_len):
            ...
            for row in range(self.args['out_res']):
                ...
                grid_points = grid_points.reshape(1,-1,3).to('cpu', dtype=torch.float32) # BILO CUDA
